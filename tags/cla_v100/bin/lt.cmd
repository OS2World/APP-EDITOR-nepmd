/*
 *      LT.CMD - V1.0 C.Langanke for Netlabs EPM Distribution Project 2002
 *
 *      Syntax: lt dir [dir [...]]
 *
 *    This program renames all files and directories of a directory tree
 *    to lowercase. The names of the directories specified as parameter
 ...
 DO UNTIL (TRUE)
    ...
    DO WHILE (DirList \= '')
       ...
       DO d = Dir.0 TO 1 BY -1
          rc = MakeLowercaseSubdirectory( Dir.d);
       END;

       rc = MakeLowercaseSubdirectory( Dir);
    END;
...
/* ========================================================================= */
MakeLowercaseSubdirectory: PROCEDURE EXPOSE (GlobalVars)
PARSE ARG Dir;

 /* rename files */
 rc = SysFileTree( Dir'\*', 'File.', 'FO');
 DO f = 1 TO File.0
    rc = MakeLowercase( File.f);
    IF (rc \= ERROR.NO_ERROR) THEN
       SAY 'Warning: cannot rename file:' File.f;
 END;

 /* rename directory itself */
 rc = MakeLowercase( Dir);
 IF (rc \= ERROR.NO_ERROR) THEN
    SAY 'Warning: cannot rename direcory:' Dir;

 RETURN( rc);

/* ========================================================================= */
MakeLowercase: PROCEDURE EXPOSE (GlobalVars)
 ARG Filename;

 /* check new name */
 NewFilename = LOWER( FILESPEC( 'N', FileName));

 /* rename file */
 'REN "'Filename'" "'NewFilename'"' Redirection;
