...
.pl bold
- SWAPBUTTON
= TRUE if pointing device buttons are swapped. Normally, the pointing device
  buttons are set for right-handed use. If this value is TRUE,
...
- CYFULLSCREEN
= Height of the client area when the window is full screen (excluding
  menu height).
- DEBUG
= FALSE indicates this is not a debug system.
- CMOUSEBUTTONS
= The number of buttons on the pointing device (zero if no pointing device
  is installed).
- POINTERLEVEL
...
*NepmdQuerySysInfo* returns the system value.
...
Executing this command will open up a virtual file and write all
[.IDPNL_EFUNC_NEPMDQUERYSYSVALUE_PARM_VALUETAG supported system values]
into it.
