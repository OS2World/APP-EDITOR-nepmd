...
** Changes:
**
**    2006-04-23  Fixed renaming of tmp ChangeLog.  [Andreas Schnellbacher]
**    2006-05-07  Disabled appending hostname to author.
**                Fixed check for empty repository name.
...
**                Schnellbacher]
**    For more recent changes see the NEPMD changelog
**
**    todo:
**          -  maybe remove repository path and show only subpathes
**          -  maybe indent "* <file>" line just by a half tabwidth
**          -  option: reenable "@hostname"
**          -  option: sort by filenames instead of date -> new .cmd
**          -  fix: "-d >date" option, all date specs are handled like "<date"
**          -  fix: file must be deleted manually before not to copy the
**             first new block only above the old contents.
**
...
edit_changelog: procedure expose (globals)
   ...
   if exists(dir''changelog_prefix) then do
      ...
      cmd = 'copy' dir''changelog_prefix '+' dir''changelog_name tempname
      '@'cmd '>nul 2>&1'
      next = stream(dir''changelog_name, 'c', 'query exists')
      if next > '' then
         call SysFileDelete next
      next = stream(dir''changelog_prefix, 'c', 'query exists')
      if next > '' then
         call SysFileDelete next
/*
      cmd = 'erase' dir''changelog_name dir''changelog_prefix
      '@'cmd '>nul 2>&1'
*/
      cmd = 'rename' tempname changelog_name
      '@'cmd '>nul 2>&1'
      '@start /f 'cvseditor dir''changelog_name
   end
   return
...
do_directory: procedure expose (globals) dateval tz text.
   ...
   if translate(right(dir, 9)) == '/CVSROOT/' then
      return
   if \is_directory(strip(dir,'t','/')) then do
      say "Not found: "dir
      return
   end
   ...
   if filelist = '*' & recursive then do
      ...
      do i = 1 to entries.0
         pdir = dir''entries.i'/'
         if translate(right(pdir, 9)) \= '/CVSROOT/' then do
            if is_directory(strip(pdir,'t','/')) then do
               say "Interim: "pdir
...
format_log: procedure expose (globals) tz text.
   ...
   do i = 1 to text.0
      parse var text.i nfilename nrev ndate ntime nauthor (SOH) nlog
      nclumpname = ''
...
*/
/*** not required while the following doesn't work:
   rootdir = directory()
   lp = lastpos( '\', rootdir)
   rootdirname = substr(rootdir, lp+1)
***/
   rest = files
   do while rest > ''
...
/*** following doesn't work:
      /* remove the path up to the current dirname */
      next = '/'translate(rootdirname)'/'
      p = pos( next, file)
      if p > 0 then
         file = substr(file, p+length(next))
***/
      call lineout changelog, indent_string'* 'file
   end
   indent = indent_string
   do forever
      i = pos(SOH, log)
...
read_entries: procedure expose (globals) entries.
   ...
   /* Don't stop on not ready because dir may be a removed dir */
   /* Entries.log is created after a single checkout */
   ...
   if stream(filename, 'c', 'open read') = 'READY:' then do
      do while lines(filename) > 0
         ...
         if dir2 \= '' then do
            /* Add entry only if still not present */
            ffound = 0
            do e = 1 to entries.0
